...
|---|---|---|
| `tapedeck add <url> [--force] [--verbose]` | yes | ingest → transcribe → archive → index for one video, or for every video of a playlist/channel URL (SPEC-cli-003; `--force` is single-video only; `--verbose` is routed whole to ingest and streams the fetch tool raw instead of SPEC-ingest-004's quiet capture; each completed video gets one stderr close-out line — title, channel, duration — per SPEC-cli-012; each video that completes is then handed to a detached wiki-filing worker when `[wiki].auto` is true, a best-effort epilogue that never changes add's exit code, counts, sweep, or return time; the worker files in sweep order, waits for a held wiki instead of skipping, accepted filings land in the wiki's log, and a failed one leaves its video unfiled for `wiki sync` — SPEC-cli-009, SPEC-cli-011) |
...
| `tapedeck wiki file <id> \| sync [--dry-run] \| lint [--json] \| rebuild [--yes] \| tend [--yes]` | yes (wiki/ only) | the prose layer: file one video into the wiki, file every unfiled video, diagnose a standing wiki, or reset and refile it. The group is handed over whole to the wiki component and the cli routes without re-deriving any of its vocabulary (SPEC-cli-009); behavior is SPEC-wiki-002 (`file`), SPEC-wiki-003 (`sync`), SPEC-wiki-004 (`lint`), SPEC-wiki-005 (`rebuild`) and SPEC-wiki-006 (`tend`), over the tree pinned by `contracts/wiki-layout.md` |
