...
it costs, and never decides the exit code.
...
`ask.librarian_command`, `ask.answerer_command`, `ffmpeg`, `home`, `fts5`, `platform` —
every one of them always present, including the ones that pass, because a diagnosis that
prints only complaints cannot tell "checked and fine" from "never looked". `--json` emits
the same checks in the same order as `[{check, status, detail}]`.
