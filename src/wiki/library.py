...
not copied. Whether there is anything to file *from* is archive's: the maintainer
reads `archive/<id>.md`, so a video without one has nothing to be read.
...
sweep can ever satisfy — a directory of the user's own, an entry whose video `rm
--media-only` reclaimed, one archive has not rendered yet — and a sweep that fails
on them can never converge. Each gets a note on stderr and is left alone.
...
def eligible(home: Path, note=None) -> list[str]:
    ...
    for path in sorted(root.iterdir()) if root.is_dir() else []:
        ...
        if not ingest.VIDEO_ID.fullmatch(video_id):
            _say(note, f"{video_id}: not a video id — skipped, it is not tapedeck's")
        elif not ingest.has_video(path):
            _say(
                note,
                f"{video_id}: no video here — skipped; the entry was reclaimed by "
                f"`tapedeck rm --media-only`, and `tapedeck add {video_id}` brings it back",
            )
        elif not archive_page(home, video_id).is_file():
            _say(
                note,
                f"{video_id}: no archive page — skipped; there is nothing to read it "
                f"from until `tapedeck add {video_id}` renders one",
            )
        else:
