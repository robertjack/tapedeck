...
probabilistic runs until the library is known to have something to say, and no answer
reaches stdout before its citations have been checked against it.
"""
...
def scope_for(home: Path, video_id: str | None) -> str | None:
    ...
    if video_id is None or video_id in library.videos(home):
        return video_id
    raise Failure(f"no video {video_id!r} in the library — try `tapedeck list`", code=2)


def librarian(home: Path, question: str, scope: str | None) -> int:
    ...
    asked = citations.ask_for(question, scope)
    answer = seams.run(command, home, asked, "librarian", cwd=home)
    ...
    problems = citations.unverified(links, videos, scope)
    if problems:
...
def main(argv=None) -> int:
    ...
    try:
        ...
        scope = scope_for(home, args.video)
        if args.fast:
        ...
        return librarian(home, asked, scope)
